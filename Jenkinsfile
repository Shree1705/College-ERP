...
pipeline {
    ...
    environment {
        DOCKERHUB_CREDENTIALS = credentials('docker_login')
        DOCKERHUB_REPO = 'divanshreevatsa/college_erp'
        BACKEND_IMAGE_TAG = "server-${BUILD_NUMBER}"
        FRONTEND_IMAGE_TAG = "client-${BUILD_NUMBER}"
        BACKEND_CONTAINER_NAME = "server-${BUILD_NUMBER}-c"
        FRONTEND_CONTAINER_NAME = "client-${BUILD_NUMBER}-c"
    }
